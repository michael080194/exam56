...
@section('content')
    <h1 class="text-center">
        {{$exam->title}}
        @can('建立測驗')
        <button type="button" class="btn btn-danger btn-sm btn-del-exam" data-id="{{ $exam->id }}">刪除</button>   
    ...
    @endif
            ...
            {{-- {{ bs()->formGroup()
                ->label('正確解答', false, 'text-sm-right')
                ->control(bs()->radioGroup('ans1', [1=>1, 2=>2, 3=>3, 4=>4])
                                ->selectedOption(1)
                                ->inline()
                                ->radioDisabled()
            ...
            <dt>
                ...
                @can('建立測驗')
                   {{-- <form action="{{route('topic.destroy', $topic->id)}}"  method="post" style="display:inline">
                    ...
                    <a href="{{route('topic.edit', $topic->id)}}" class="btn btn-warning btn-sm">編輯</a>
                    （答案{{$topic->ans}}）
                @endcan
                {{ bs()->badge()->text($key+1) }}
                {{$topic->topic}}
